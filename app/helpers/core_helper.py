...
from models.model_db import Impressora, Historico_Recarga
...
def get_all_printers_recarga_datas(session: Session) -> dict:
    ...
    """
    result = {printer.id: _extract_datas_recarga(printer.id, session) 
              for printer in _get_printers(session)}
...
def _calculate_next_recharge(datas_recarga: list, type: str = "media") -> str:
    ...
    datas = [
        data if isinstance(data, (datetime, date))
        else datetime.strptime(data, "%d/%m/%Y")
